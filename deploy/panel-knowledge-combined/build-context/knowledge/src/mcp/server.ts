...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // Team 上下文环境变量（由客户端 agent 配置 MCP 时注入）
  // TDAI_SERVICE_ID: 实例 ID，对应 x-tdai-service-id header
  // TDAI_USER_KEY: 用户密钥，对应 x-tdai-user-key header
  ...
  log.info(`MCP server starting, API URL: ${baseUrl}, serviceId: ${process.env.TDAI_SERVICE_ID || "(none)"}`);
