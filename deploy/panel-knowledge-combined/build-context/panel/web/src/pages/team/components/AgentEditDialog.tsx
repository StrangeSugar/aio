...
import { agentsApi } from '@/lib/teamApi';
...
export default function AgentEditDialog({
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      Promise.resolve({ status: 'fulfilled', value: { items: [] } }),
    ])
      .then(([_skillResult, knowledgeResult, _chatResult]) => {
        if (cancelled) return;

        const skillItems: never[] = [];
        const nextSkillItems = skillItems.map((s: { skill_id: string; name: string }) => ({
          key: s.skill_id,
          title: s.name || s.skill_id,
        }));
        setRealSkillItems(nextSkillItems);
        setSkills(nextSkillItems.map((s: { key: string }) => s.key));
        ...
        const chatItems: Array<{ id: string }> = [];
        const nextChatMemories = Array.from(
          new Set([selfChatMemoryId, ...chatItems.map((it: { id: string }) => it.id)]),
        );
