/**
 * /api/v1/knowledge/wiki/* —— Panel Wiki 业务路由（stateless，MCP 接入版）。
 *
 * 实现冻结契约 docs/api/knowledge-panel-api.md §2.0 Wiki 最小端点集，透传
 * HttpKnowledgeClient → KS /v3/wiki/*。
 *
 * 门控：
 *   - 带 team_id 的端点（list/create/raw/write）→ 要求有效 caller；
...
export function registerKnowledgeWikiRoutes(api: Hono, deps: PanelDeps): void {
  ...
  // W1 create — team 门控；KS create → 拿 wiki_id
  ...
  // W5 delete — 删 KS 侧资源（不再通过 MemoryProxy 级联删除）
  api.post('/knowledge/wiki/delete', mw, async (c) => {
    ...
    const kc = deps.knowledgeClientFactory(ctx.instanceId);
    return runKs(c, async () => {
      return await kc.wikiDelete(wikiIds);
    });
  });
